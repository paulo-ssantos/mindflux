#  Mindflux Admin Template for Nuxt 3  

Visit [Mindflux](https://mindflux.vercel.app) website for a live preview. 
 
### Getting Started
Mindflux is an application template for Vue based on the [Vue CLI](https://cli.vuejs.org/) that provides out-of-the-box standard
...
That is it, you may now start with the development of your application using the Mindflux template.</p>
...
### Structure
Mindflux consists of 2 main parts; the application layout and the resources. **app.vue** inside src folder is the main component containing the template for the base layout whereas required resources such as SASS structure for the layout are placed inside the **src/assets/** folder.</p>
...
### Dependencies
Dependencies of Mindflux are listed below and needs to be added to package.json.
...
### PrimeVue Theme
Mindflux uses the free Saga, Arya and Vela themes which are distributed within PrimeVue, however it can be used with any PrimeVue theme as well such as material, tailwind and bootstrap as layout colors are derived from the theme used via CSS variables.
